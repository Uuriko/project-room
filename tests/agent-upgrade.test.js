...
for (const [version, baseline] of [[8, v8ConnectionBaseline], [9, v9TextBaseline], [10, v10CharterBaseline], [11, v11ReplyBaseline]]) test(`genuine v${version} data upgrades atomically; old writers cannot write v12`, async t => {
  ...
  assert.equal(observedVersion, 12, "fault was injected after installing the new writer");
  ...
  assert.throws(() => cached.run(f.owner.session.account.id), /project_room_writer_v(?:9|10|11|12)|unsupported database writer/);
  assert.throws(() => new OldStore(f.filename), /newer than this service/);
  current.createAccount("after-v12-upgrade");
  assert.equal(current.account("after-v12-upgrade").active, true);
  assert.equal(auditRecovery(current).schemaVersion, 12);
