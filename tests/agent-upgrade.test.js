...
for (const [version, baseline] of [[8, v8ConnectionBaseline], [9, v9TextBaseline], [10, v10CharterBaseline], [11, v11ReplyBaseline], [12, v12HelpBaseline], [13, v13OfferBaseline], [14, v14InboxBaseline], [15, v15AdoptionBaseline]]) test(`genuine v${version} data upgrades atomically; old writers cannot write v16`, async t => {
  ...
  assert.equal(observedVersion, 16, "fault was injected after installing the new writer");
  ...
  assert.throws(() => cached.run(f.owner.session.account.id), /project_room_writer_v(?:9|10|11|12|13|14|16)|unsupported database writer/);
  assert.throws(() => new OldStore(f.filename), /newer than this service/);
  current.createAccount("after-v16-upgrade");
  assert.equal(current.account("after-v16-upgrade").active, true);
