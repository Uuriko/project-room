...
for (const [version, baseline] of [[8, v8ConnectionBaseline], [9, v9TextBaseline], [10, v10CharterBaseline], [11, v11ReplyBaseline], [12, v12HelpBaseline], [13, v13OfferBaseline], [14, v14InboxBaseline], [15, v15AdoptionBaseline], [16, v16SendBaseline], [17, v17EmailBaseline], [18, v18EmailSourceBaseline], [19, v19EmailExcerptBaseline], [20, v20ReplyJournalBaseline], [21, v21ReplyReviewBaseline]]) test(`genuine v${version} data upgrades atomically; old writers cannot write v22`, async t => {
  ...
  assert.equal(observedVersion, 22, "fault was injected after installing the new writer");
  ...
  assert.throws(() => cached.run(f.owner.session.account.id), /project_room_writer_v(?:9|10|11|12|13|14|17|22)|unsupported database writer/);
  assert.throws(() => new OldStore(f.filename), /newer than this service/);
  current.createAccount("after-v22-upgrade");
  assert.equal(current.account("after-v22-upgrade").active, true);
  assert.equal(auditRecovery(current).schemaVersion, 22);
  if (version >= 21) {
