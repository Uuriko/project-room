...
for (const [version, baseline] of [[8, v8ConnectionBaseline], [9, v9TextBaseline]]) test(`genuine v${version} data upgrades atomically; old writers cannot write v10`, async t => {
  ...
  AgentConnections.prototype.verifyHistory = function () { assert.equal(this.store.storagePlatform.version(this.store.db), 10); throw new Error("synthetic final failure"); };
  ...
  assert.throws(() => cached.run(f.owner.session.account.id), /project_room_writer_v(?:9|10)|unsupported database writer/);
  assert.throws(() => new OldStore(f.filename), /newer than this service/);
  current.createAccount("after-v10-upgrade");
  assert.equal(current.account("after-v10-upgrade").active, true);
  assert.equal(auditRecovery(current).schemaVersion, 10);
  if (version === 9) {
    const check = store => {
