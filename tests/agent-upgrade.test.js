...
for (const [version, baseline] of [[8, v8ConnectionBaseline], [9, v9TextBaseline], [10, v10CharterBaseline], [11, v11ReplyBaseline], [12, v12HelpBaseline], [13, v13OfferBaseline], [14, v14InboxBaseline], [15, v15AdoptionBaseline], [16, v16SendBaseline], [17, v17EmailBaseline]]) test(`genuine v${version} data upgrades atomically; old writers cannot write v18`, async t => {
  ...
  assert.equal(observedVersion, 18, "fault was injected after installing the new writer");
  ...
  assert.deepEqual(auditRecovery(current).tables.filter(row => !row.table.startsWith("private_email_") && (version >= 15 || !row.table.startsWith("private_inbox_")) && (version >= 9 || !row.table.startsWith("agent_connection"))), before.tables);
  assert.deepEqual(current.email.verify(), { connections: 0, folders: 0, sources: 0 });
  assert.equal(current.authenticate(f.keys.agent).member.id, "agent");
  assert.throws(() => cached.run(f.owner.session.account.id), /project_room_writer_v(?:9|10|11|12|13|14|17|18)|unsupported database writer/);
  assert.throws(() => new OldStore(f.filename), /newer than this service/);
  current.createAccount("after-v18-upgrade");
  assert.equal(current.account("after-v18-upgrade").active, true);
  assert.equal(auditRecovery(current).schemaVersion, 18);
