...
test("Bearer pri_ exposes room tools and keeps command receipts", async t => {
  ...
  const outsiderIdentity = store.identities.create("MCP outsider");
  ...
  const peerLinked = store.identities.link(owner.secret, created.roomId, {
    identityId: peer.identityId, displayName: "MCP peer", permissions: []
  });
  assert.equal(peerLinked.memberId, peer.identityId);
  ...
  const outsider = await call(origin, "room_list_events", { roomId: created.roomId }, outsiderIdentity.secret);
  ...
  const hiddenBoard = await call(origin, "room_read_board", { roomId: created.roomId }, outsiderIdentity.secret);
