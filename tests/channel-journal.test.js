...
test("schema: a v28 file from before the journal opens read-only and gains the table on a writable open; a partial schema is refused", t => {
  const f = fixture(t);
  ...
  raw.close();
  // Read-only never migrates: an older backup is still a valid v28 file.
  ...
  assert.equal(f.store.storagePlatform.version(f.store.db), 28);
