...
test('equal independently captured data is not permission to reopen; all 63 tables are compared', async t => {
  const f = await fixture(t), reference = await f.capture(), report = compare(f.older, reference);
  assert.equal(report.status, 'no_stored_differences'); assert.equal(report.tables.length, 63); // +3: agent_api_keys, agent_directory_cards, agent_webhook_subs (RC-2026-09-18-010); +5: collab_assignments, collab_notes, collab_draft_locks, collab_approvals, collab_routing_events (RC-2026-09-18-011)
