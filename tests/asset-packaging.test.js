...
test('asset build produces and refreshes exactly the allowlisted application files', async t => {
  ...
  assert.equal(config.build.command, 'node ../scripts/stamp-version.mjs && node ../scripts/build-capabilities.mjs && node build-assets.mjs', 'deploy stamps the committed revision before building exact assets');
});
