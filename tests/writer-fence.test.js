...
test("schema upgrade preserves existing records and fences a previously opened connection", t => {
  ...
  try {
    assert.equal(current.db.prepare("PRAGMA user_version").get().user_version, 24);
    ...
    assert.throws(() => legacyStatement.run("fixture-account"), /project_room_writer_v24|unsupported database writer/);
...
test("failure after fence installation rolls back its schema marker and all triggers", t => {
  ...
  try {
    ...
    assert.equal(inspected.prepare("SELECT count(*) n FROM sqlite_master WHERE type='trigger' AND name LIKE 'writer_v24_%'").get().n, 0);
...
test("read-only audit does not upgrade a pre-fence database", t => {
  const filename = legacyFixture(t);
  assert.throws(() => new RoomStore(filename, { readOnly: true }), /requires schema v24/);
  const inspected = new DatabaseSync(filename, { readOnly: true });
...
test("the v9 migration preserves v6 guards while retiring pre-open v6 writers", t => {
  ...
  try {
    assert.equal(current.db.prepare("PRAGMA user_version").get().user_version, 24);
    assert.throws(() => oldWrite.run(), /project_room_writer_v24|unsupported database writer/);
...
test("v7 to v9 preserves every existing table row and retires a pre-open v7 connection", t => {
  ...
  try {
    ...
    assert.throws(() => cached.run(), /project_room_writer_v24|unsupported database writer/);
