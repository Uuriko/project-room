...
test("the v1 projection, checkpoint, and version marker roll back together", () => {
  ...
  try {
    ...
    store.db.exec("DROP TABLE agent_connection_operations; DROP TABLE agent_connections; DROP TABLE private_reminder_commands; DROP TABLE private_reminders; DROP TABLE projection_checkpoints; PRAGMA user_version=1");
...
test("fresh databases use schema v13 and every unsupported schema fails closed without mutation", () => {
  ...
  store.db.exec("PRAGMA journal_mode=DELETE; PRAGMA user_version=14");
  store.close();
