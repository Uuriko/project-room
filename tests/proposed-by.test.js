...
test("v1 upgrades checkpoint a conservative projection and strictly replay the v2 tail", () => {
  ...
  store = new RoomStore(filename);
  try {
    assert.equal(store.db.prepare("PRAGMA user_version").get().user_version, 32);
...
test("the v1 projection, checkpoint, and version marker roll back together", () => {
  ...
  try {
    ...
    store.db.exec("DROP TABLE room_attachments; DROP TABLE private_email_folders; DROP TABLE private_email_commands; DROP TABLE private_email_connections; DROP TABLE private_inbox_drafts; DROP TABLE private_inbox_versions; DROP TABLE private_inbox_sources; DROP TABLE private_inbox_commands; DROP TABLE agent_connection_operations; DROP TABLE agent_connections; DROP TABLE private_reminder_commands; DROP TABLE private_reminders; DROP TABLE projection_checkpoints; PRAGMA user_version=1");
...
test("fresh databases use schema v32 and every unsupported schema fails closed without mutation", () => {
  ...
  store.db.exec("PRAGMA journal_mode=DELETE; PRAGMA user_version=33");
  store.close();
  assert.throws(() => new RoomStore(filename), /schema is newer/);

  let raw = new DatabaseSync(filename);
  assert.equal(raw.prepare("PRAGMA user_version").get().user_version, 33);
