...
test("fresh databases use schema v18 and every unsupported schema fails closed without mutation", () => {
  ...
  store.db.exec("PRAGMA journal_mode=DELETE; PRAGMA user_version=19");
  store.close();
  assert.throws(() => new RoomStore(filename), /schema is newer/);

  let raw = new DatabaseSync(filename);
  assert.equal(raw.prepare("PRAGMA user_version").get().user_version, 19);
