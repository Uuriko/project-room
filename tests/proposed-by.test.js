...
test("v1 upgrades checkpoint a conservative projection and strictly replay the v2 tail", () => {
  ...
  store = new RoomStore(filename);
  try {
    assert.equal(store.db.prepare("PRAGMA user_version").get().user_version, 33);
...
test("fresh databases use schema v33 and every unsupported schema fails closed without mutation", () => {
  ...
  store.db.exec("PRAGMA journal_mode=DELETE; PRAGMA user_version=34");
  store.close();
  assert.throws(() => new RoomStore(filename), /schema is newer/);

  let raw = new DatabaseSync(filename);
  assert.equal(raw.prepare("PRAGMA user_version").get().user_version, 34);
