...
test("online capture preserves all 96 tables, identity boundaries and exact retries through recovery and restart", async t => {
  const f = fixture(t);
  ...
  assert.equal(before.rooms, 2); assert.equal(before.tables.length, 96,
