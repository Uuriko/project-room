...
test("online capture preserves all 117 tables, identity boundaries and exact retries through recovery and restart", async t => {
  const f = fixture(t);
  ...
  f.store.db.prepare(`INSERT INTO thread_mutes(room_id,member_id,thread_id,created_at)
    ...
    .run(f.now());
  // Seed one operator-controls row so the capture comparison covers
  // agent_operator_controls (operator prerequisites slice 1/3, PR #928).
  f.store.db.prepare(`INSERT INTO agent_operator_controls(room_id,member_id,spend_cap_cents,spend_period_days,killed_at,autonomy_tier,sandboxed,updated_at,updated_by)
    VALUES('commons','agent',5000,30,NULL,'t2_standard',0,?, 'owner')`)
    .run(f.now());
