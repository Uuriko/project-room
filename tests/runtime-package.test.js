...
test("uncommitted candidate packages cold in an isolated synthetic commit, including request observer dependencies", t => {
  ...
  const receipt = createRuntimePackage({ ...candidate, destination }); assert.equal(receipt.files, 196); // +2 SLA hook files +2 quarantine review files +1 shadow instrumentation +6 sms/messenger channel files +1 quarantine-review-coverage +7 Lane D agent plug-in modules +5 Lane C collab pure modules +2 Lane C collab store/routes (RC-2026-09-18-011) +1 SLA dashboard module
  const program = `
