...
gmailPackageFiles.push(...['telegram-runtime','telegram-connection-registry','telegram-connections','telegram-receiver','telegram-inbox-import','telegram-receive-queue','telegram-receive-tick','telegram-bot-reader','twilio-message-reader','twilio-inbox-import','slack-event-reader'].map(name=>`server/${name}.mjs`));
...
test("uncommitted candidate packages cold in an isolated synthetic commit, including request observer dependencies", t => {
  ...
  const receipt = createRuntimePackage({ ...candidate, destination }); assert.equal(receipt.files, 125);
  const program = `
