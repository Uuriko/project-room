...
test("uncommitted candidate packages cold in an isolated synthetic commit, including request observer dependencies", t => {
  ...
  const receipt = createRuntimePackage({ ...candidate, destination }); assert.equal(receipt.files, 168); // +8 vs pre-handoff: inbox-handoff.mjs (task 23), inbox-stitch.mjs, inbox-stitch-store.mjs (task #19), sla-urgent-notify.mjs (tasks 24/34/35), spam-quarantine-journal.mjs, inbox-spam.mjs, notify-prefs.mjs, inbox-import-guards.mjs (tasks 32/34)
  const program = `
