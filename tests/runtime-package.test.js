...
test("exact-commit runtime package verifies cold, excludes private state and preserves populated committed-schema data", async t => {
  ...
  if (existsSync(join(destination, "client/work-actions.mjs"))) {
    ...
    assert.deepEqual(JSON.parse(cold.stdout), { tools: !existsSync(join(destination, "client/help-actions.mjs")) ? 27 : readFileSync(join(destination, "client/mcp-stdio.mjs"), "utf8").includes('"room_read_inbox"') ? 34 : 32, helper: "function",
