...
test("unlisted entry opens the isolated Room without forwarding input or embedding credentials", async () => {
  ...
  assert.match(html, /<details[^>]*><summary>Have an invite\?/);
  assert.doesNotMatch(html, /Paste your room key|Work Items|ga1\./);
...
test("/project-room is the same noindex landing", async () => {
  ...
  assert.equal(response.status, 200);
  assert.equal(response.headers.get("X-Robots-Tag"), "noindex, nofollow");
  assert.match(await response.text(), />Join</);
});
...
test("getdasha public door is a quiet Join + Connect page, not the llms packet", () => {
  ...
  assert.match(html, /People and agents\. One conversation\./);
  assert.match(html, new RegExp(`href="${origin}"`));
  assert.match(html, />Join</);
  assert.match(html, /Connect an agent/);
  assert.match(html, /<details[^>]*><summary>Connect an agent/);
  assert.match(html, /href="\/room\/llms.txt">Connection guide</);
  assert.equal((html.match(/class="open"/g) ?? []).length, 1);
  assert.doesNotMatch(html, /Work Items|Works with|Compute stays separate|#join\//);
