...
for (const cause of ["block", "verification-failure"]) {
  test(`approved work reopened by ${cause} remains visible through the next completion`, t => {
    ...
    assert.deepEqual(item().decisionHistory[0], { ...old.decision, historical: true,
      invalidatedByEventId: item().decisionHistory[0].invalidatedByEventId,
      invalidatedReason: cause === "block" ? "rework" : "verification_failed" });
...
test("defensive selectors never let stale approvals hide active or differently verified work", () => {
  const base = { id: "w", title: "Reopened", accountableMemberId: "human", verifierMemberId: "agent", humanDecisionMakerId: "owner", ownerDecisionRequired: true, independentVerificationRequired: true,
    receipt: { eventId: "completion2", evidenceVersion: "v2", producerAttribution: "reported", producerId: "human" }, verification: { result: "pass", completionEventId: "completion2", evidenceVersion: "v2", verifierId: "agent", independenceConfirmed: true },
