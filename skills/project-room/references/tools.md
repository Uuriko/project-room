...
| `room_begin_work` | accept, claim, then start | One Begin for selected work. Confirms API identity only. working is the Room work state, not a host start. An unknown stage keeps its request id; a different scope stops and shows the current claim. The browser records the existing Room action and does not invoke Begin. Write mode needs repository, ref, paths, and expiresAt; those are not guessed. |
