...
const HASH_OPERATOR = /^idp-[a-f0-9]{64}$/;

export function isNamedOperatorId(value) {
  if (typeof value !== 'string' || value.includes('@')) return false;
  return HASH_OPERATOR.test(value) || validId(value);
}
...
// Fail closed when ROOM_PRODUCTION=1. Named local operator; no identity provider.
export function assertProductionReady(env, origin, { ship = openJoinContract().ship } = {}) {
  if (env.ROOM_PRODUCTION !== '1') {
    return { production: false, providerAuth: null, operatorAccountId: null };
  }
  const operatorAccountId = env.ROOM_OPERATOR_ACCOUNT_ID;
  if (!isNamedOperatorId(operatorAccountId || '')) {
    throw new Error('ROOM_PRODUCTION requires ROOM_OPERATOR_ACCOUNT_ID as a local account/member id');
  }
  if (ship !== false) throw new Error('ROOM_PRODUCTION forbids shipping public MCP join');
  return { production: true, providerAuth: null, operatorAccountId };
}
