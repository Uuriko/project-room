...
function project(message, draftId = null) {
  ...
  return { id: message.id, threadId: message.threadId, draftId, from: header(message, 'From'), replyTo: header(message, 'Reply-To'), to: header(message, 'To'), cc: header(message, 'Cc'), bcc: header(message, 'Bcc'), subject: header(message, 'Subject'), date: header(message, 'Date'), labels: message.labelIds ?? [], snippet: message.snippet ?? '',
...
export class GmailActions {
  ...
  async run(token, binding, input) {
    ...
    const { action } = input;
    const allowed = ['action', 'requestId', 'id', 'draftId', 'expectedMessageId', 'replyId', 'folder', 'query', 'pageToken', 'to', 'cc', 'bcc', 'subject', 'body'];
