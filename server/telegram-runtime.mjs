...
export function createTelegramRuntime({env=process.env,store,fetchImpl=fetch,sourceRoot=fileURLToPath(new URL('../',import.meta.url))}) {
  ...
  let rdb,qdb,registry,queue,key;
  try {
    ...
    const connections=createTelegramConnections({store,registry,connections:[{...binding,queue}],fetchImpl});
    let closed=false;
    return {connections,close(){if(closed)return;closed=true;queue.key.fill(0);registry.close();qdb.close();rdb.close();}};
  }catch{queue?.key.fill(0);registry?.close();qdb?.close();rdb?.close();fail();}
  finally{key?.fill(0);}
}
