...
export function createRoomServer({ store, origin, assetRoot = new URL("../", import.meta.url), streamInterval = STREAM_INTERVAL_DEFAULT_MS, streamQueueCap = 65536, trustedLocalProxy = false,
  ...
  // Self-serve access requests live on the store (server/store.mjs owns the
  // schema); the routes below use store.accessRequests.
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (url.pathname === "/api/access-requests" && req.method === "POST") {
        ...
        return json(res, 201, store.accessRequests.request(data.roomId, data));
      }
      ...
      if (accessStatusMatch && req.method === "GET") {
        ...
        return json(res, 200, store.accessRequests.status(pathId(accessStatusMatch[1]), identityId));
      }
      ...
      if (route === "access-requests" && req.method === "GET") {
        ...
        return json(res, 200, { roomId, requests: store.accessRequests.list(selected.token, roomId, { status }, fence) });
      }
      if (route === "access-decide" && req.method === "POST") {
        ...
        return json(res, 200, store.accessRequests.decide(selected.token, roomId, accessRequestId, data, fence));
      }
