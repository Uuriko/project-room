...
export function createRoomServer({ store, origin, assetRoot = new URL("../", import.meta.url), streamInterval = STREAM_INTERVAL_DEFAULT_MS, streamQueueCap = 65536, trustedLocalProxy = false,
  ...
  // Self-serve access requests: the schema lives in the module and is applied
  // here until store.mjs owns it (deferred while the read/unread lane holds
  // store.mjs). The instance is request-scoped to this server.
