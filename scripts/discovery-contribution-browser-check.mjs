...
for (const touch of [false, true]) test(`discovery to contribution ${touch ? 'touch' : 'desktop'}: reuse, fresh authority and changed evidence`, { timeout: 60000 }, async t => {
  ...
  for (const id of ['require-verification', 'require-decision']) assert.equal(await page.locator(`#${id}`).isChecked(), true, 'reuse starts from the approved checked-by-default policy');
