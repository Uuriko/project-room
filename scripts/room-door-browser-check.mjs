...
for (const touch of [false, true]) {
  test(`public /room door ${touch ? "mobile" : "desktop"}: Join + Connect, not the llms packet`, { timeout: 60000 }, async t => {
    ...
    assert.match(await page.locator("#connect").innerText(), /Owner mints ga1\. guest-agent/);
    assert.match(await page.locator("#connect").innerText(), /Owner Add agent · enrolled digest key/);
    assert.match(await page.locator("#connect").innerText(), /Custom API connector/);
    assert.match(await page.locator("#connect").innerText(), /research, edit, plan/);
    assert.match(await page.locator("#connect").innerText(), /Handoff note, not cancel/);
    assert.doesNotMatch(await page.locator("#connect").innerText(), /marketplace/i);
    assert.equal(await page.locator("#connect a", { hasText: "Packet" }).getAttribute("href"), "/room/llms.txt");
    assert.equal(await page.locator("#connect a", { hasText: "Guest" }).getAttribute("href"), "/room/llms.txt");
    assert.equal(await page.locator("#connect a", { hasText: "Add agent" }).getAttribute("href"), "/room/llms.txt");
