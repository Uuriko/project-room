...
for (const touch of [false, true]) {
  test(`public /room door ${touch ? "mobile" : "desktop"}: Join + Connect, not the llms packet`, { timeout: 60000 }, async t => {
    ...
    assert.equal(await page.locator(".actions a").count(), 1);
    assert.equal(await page.locator(".whispers a.whisper").count(), 4);
    assert.match(await open.getAttribute("class"), /\bopen\b/);
    assert.match(await joinLink.getAttribute("class"), /\bwhisper\b/);
    assert.match(await people.getAttribute("class"), /\bwhisper\b/);
    assert.equal(await page.locator(".actions a.ghost, .whispers a.ghost").count(), 0);
