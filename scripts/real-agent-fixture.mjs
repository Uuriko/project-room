...
// This harness creates assignments, never the producer's answer or review verdict.
...
if (mode === 'publish') {
  ...
  seed[0].data.title = 'Dasha Compute — Agent Test';
  ...
  for (const memberId of ['owner', 'producer', 'reviewer']) {
    ...
    writeFileSync(filename, JSON.stringify({ fixture: 'project-room-real-agent-v1', origin, roomId: 'commons', memberId, token: keys[memberId], artifactOrigin, artifactDirectory, artifactCaFile, workItemId: 'dasha-bridge-acceptance' }), { mode: 0o600, flag: 'wx' });
...
} else throw new Error('Use serve, or publish PRODUCER-CONFIG ARTIFACT.md');
