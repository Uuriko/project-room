...
test("large Unicode proposal is a correctable refusal, not an uncertain retry", { timeout: 30000 }, async t => {
  const f = await setup(t), { page } = f; await f.open(); await f.people();
  // The product caps the form inputs with maxlength, so a real user can never submit a
  // >16KB proposal through this form — but API callers can, and the server must still
  // refuse it as a correctable 413 (too_large) rather than an uncertain retry. Lift the
  // caps for this journey so the oversized payload actually reaches the server.
  await f.title.evaluate(node => node.removeAttribute("maxlength"));
  await f.done.evaluate(node => node.removeAttribute("maxlength"));
