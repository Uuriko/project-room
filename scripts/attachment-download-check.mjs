...
for (const mobile of [false, true]) test(`verified file download ${mobile ? 'mobile' : 'desktop'}`, async t => {
  ...
  const retry = page.locator('#composer-files').getByRole('button', { name: 'Retry retry.txt', exact: true });
  await retry.waitFor();
  assert.match(await page.locator('#composer-files').textContent(), /Failed to fetch|Upload failed|fetch/i);
  await retry.focus(); await retry.press('Enter');
  await page.locator('#composer-files').getByText('Ready', { exact: true }).waitFor();
  assert.equal(await page.evaluate(() => document.activeElement.getAttribute('aria-label')), 'Remove retry.txt');
  assert.equal(await page.locator('#file-upload-status').textContent(), 'retry.txt: Ready');
  ...
  await page.locator('#composer-files').getByRole('button', { name: 'Remove retry.txt', exact: true }).click();
  assert.equal((await removed).status(), 200);
  assert.equal(await page.evaluate(() => document.activeElement.id), 'attach-file');
