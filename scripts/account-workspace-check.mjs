...
test("account-only other-tab replacement clears a held private read and navigation metadata", { timeout: 35000 }, async t => {
  ...
  await p.route("**/api/inbox/sources/private?view=email-excerpt-v1", async route => { const response = await route.fetch(); started(); await held; await route.fulfill({ response }); });
