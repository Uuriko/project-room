...
for (const touch of [false, true]) {
  ...
  test(`quiet design ${label}: disclosure, keyboard sending, modal focus and reflow`, { timeout: 60000 }, async t => {
    ...
    await page.locator("#new-work-button").click();
    assert.equal(await page.locator("#require-verification").isChecked(), true, 'close resets to the approved checked-by-default policy');
    assert.equal(await page.locator("#require-decision").isChecked(), true, 'close resets to the approved checked-by-default policy');
