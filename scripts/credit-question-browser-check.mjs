...
for (const mobile of [false, true]) test(`credit question ${mobile ? "mobile" : "desktop"}: preserved drafts, earlier result, exact retry and MCP answer`, { timeout: 60000 }, async t => {
  ...
  // The work timeline re-renders on composer mode changes and resets <details>
  // to closed, so open the card details right before each ask-credit click.
  const openCardDetails = async (button = "[data-ask-credit]") => {
    if (!await card.locator(".work-details").evaluate(node => node.open)) await card.locator(".work-details > summary").click();
    await card.locator(button).waitFor({ state: "visible" });
  };
  ...
  await openCardDetails(); await card.locator("[data-ask-credit]").click();
  ...
  await page.locator("#request-exit").click(); await openCardDetails(); await card.locator("[data-ask-credit]").click();
  ...
  await openCardDetails("[data-resume-credit]"); await card.locator("[data-resume-credit]").click();
  assert.equal(await input.inputValue(), "Who wrote the original draft, and which parts used AI?");
