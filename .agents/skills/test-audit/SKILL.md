...
<!--
Project Room copy of OpenClaw's test-audit skill.
Source: https://github.com/openclaw/openclaw/blob/main/.agents/skills/test-audit/SKILL.md
Copyright (c) 2026 OpenClaw Foundation
SPDX-License-Identifier: MIT

This file is MIT-licensed upstream material. Project Room itself stays
Apache-2.0. The MIT license text is in LICENSE next to this file, and the
notice is also recorded in the repository THIRD_PARTY.md. The "Project Room"
section at the end is a local adaptation; the rest is the upstream skill.
-->
...
Upstream validation names OpenClaw's Vitest runner (`scripts/run-vitest.mjs`,
`$openclaw-testing`, `$crabbox`, `$autoreview`, `scripts/pr`). Those commands
are not in this repository. Here:

- Run tests with Node 24.19 or newer: `scripts/test-env.sh npm test`
  (`TMPDIR` must be the worktree `.tmp/`, not the shared `/tmp` tmpfs).
- Do not edit source or tests while that suite is running in the checkout.
- Lint and the contract gate: `npm run lint` and `npm run check`.
- Workers checks: `pnpm --dir cloudflare install --frozen-lockfile --ignore-scripts`,
  then `pnpm --dir cloudflare test`.
- A platform double (Durable Object, KV, R2, fetch, or service binding) must
  throw on unknown methods. A stub that accepts every call hides a missing
  RPC method. That is how the cron Durable Object bug passed this suite.
- An audit may drop line or branch coverage by about one point at most, and
  must not drop coverage of the auth paths in `cloudflare/room.mjs`.
- Keep security, auth, Bond and DM consent, protocol and MCP contracts,
  migrations, and storage tests even when they look like source inspection.
