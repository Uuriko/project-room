...
export function producerKnown(item) {
...
export function verificationSatisfied(item) {
  return !item.independentVerificationRequired ||
    (item.verification?.result === "pass" && matchesReceipt(item.verification, item.receipt) &&
      ...
      item.verification.verifierId === item.verifierMemberId &&
      producerKnown(item) && item.receipt.producerId !== item.verification.verifierId);
}
...
export function workStatus(item) {
  ...
  if (item.state === S.COMPLETED) {
    ...
    if (item.independentVerificationRequired && !producerKnown(item))
      return { label: "Producer unknown", next: "Identify who produced the submitted result.", owner: item.accountableMemberId };
    if (item.independentVerificationRequired && item.receipt.producerId === item.verifierMemberId)
      return { label: "Reviewer conflict", next: "Resolve the producer and verifier overlap before review.", owner: item.accountableMemberId };
