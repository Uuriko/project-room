...
export function installGmailWorkspace({ api, ownerKey }) {
  ...
  const button = (text, fn, parent) => { const b = document.createElement('button'); b.type = 'button'; b.textContent = text; b.addEventListener('click', fn); parent.append(b); return b; };
  ...
  function compose(mode = 'new', m = null) {
    ...
    if (m) {
      if (mode === 'draft') { editing = { draftId: m.draftId, expectedMessageId: m.id }; for (const k of ['to', 'cc', 'bcc', 'subject', 'body']) form.elements[k].value = m[k]; }
        ...
        const recipients = emails(m.from).filter(e => e.toLowerCase() !== status.address.toLowerCase()); form.elements.to.value = recipients.join(', ');
  ...
  async function submit(action) {
    ...
    try {
      ...
      if (!current()) return;
      if (error.code && !['network_error', 'request_failed'].includes(error.code)) { pending = null; lock(false); d('[data-result]').textContent = errorText(error); }
  ...
  return {
    setStatus(value) { status = value; $('[data-address]').textContent = value.address ?? ''; $('[data-compose]').disabled = !value.canWrite; },
    ...
    reset() { generation++; listTurn++; readTurn++; status = selected = editing = pending = null; busy = dirty = false; dialog.close(); form.reset(); root.hidden = true; panel.classList.remove('gmail-active'); $('[data-list]').replaceChildren(); $('[data-reader]').replaceChildren(); $('[data-address]').textContent = ''; notice(''); }
  };
}
