...
export class ReturnBrief {
  constructor(client, { onChange = () => {}, onError = () => {}, onReconciliationFailure = () => {} } = {}) {
    Object.assign(this, { client, onChange, onError, onReconciliationFailure });
    this.chain = null;
    this.brief = null;
    this.message = "";
  ...
  owner() {
    ...
    return session ? { generation, room: session.roomId, viewer: session.member.id,
      account: session.account?.id, authEpoch: session.account?.authEpoch,
      sessionBinding: session.sessionBinding } : null;
  }
  owns(chain) {
  ...
  reset() { this.chain = null; this.brief = null; this.message = ""; this.reconciliationRequired = false; this.onChange(); }
  refresh({ reconciling = false } = {}) {
    const owner = this.owner();
    if (!owner) { this.reset(); return Promise.resolve(); }
    // Reopening the panel must not replace a durable acknowledgement in flight.
    if (this.owns(this.chain) && this.chain.acknowledging) return Promise.resolve();
    this.brief = null;
    const chain = this.chain = { owner, loading: true, reconciling };
  ...
  fetchPage(chain, continuation = null) {
    chain.flight = (async () => {
      try {
        ...
        if (chain.reconciling) this.reconciliationFailed(error);
        else this.message = "Catch-up could not load. Retry when connected.";
        this.onError(error);
      } finally {
  ...
  acknowledge(horizon = this.brief?.history.evaluatedThrough) {
    ...
    return (async () => {
      ...
      try {
        ...
        if (!this.owns(chain)) return;
        chain.acknowledging = false;
        await this.refresh({ reconciling: true });
      } catch (error) {
        if (!this.owns(chain)) return;
        if (saved) this.reconciliationFailed(error);
        else this.message = "Saving could not be confirmed. Refresh to check your position before retrying.";
        this.onError(error);
      } finally {
