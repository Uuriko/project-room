...
// invitee who chooses Google/GitHub OAuth loses the invitation through the
// round-trip: the post-login page refreshes to /?room=X or /?account=1
// without the hash, and the in-memory invitation object is gone with the
// old document. While the invitation dialog is live, the secret is mirrored
// into sessionStorage; boot restores it one-shot when landing without a
// room context, so the dialog re-opens after OAuth sign-in.
//
...
// Mirror the live invitation secret so an OAuth navigation cannot drop it.
