...
const client = new RoomClient({
  ...
  onAccessEnded() {
    ...
    // Leaving (or losing) a room must not leave its modal settings dialog open
    // over the account workspace; nothing behind it would be clickable.
...
$("#room-archive-button").addEventListener("click", async () => {
  ...
  try {
    ...
    if (sameSession(generation, roomId, memberId)) {
      ...
      // The room's settings are stale once it is archived; dismiss the modal so
      // the reader lands back in the room instead of behind a dialog.
      if ($("#settings-dialog")?.open) $("#settings-dialog").close();
    }
  } catch (error) {
...
function renderMessages() {
  ...
  messages.forEach((message, index) => {
    ...
    // Positioning happens once below in merged chronological order: inserting
    // here by message index would transiently misplace nodes whenever work
    // cards interleave, and the corrective move collapses text selections.
    ordered.push(node);
  });
