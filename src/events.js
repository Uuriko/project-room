...
function validateEnvelope(incoming) {
  ...
  for (const [key, value] of Object.entries(incoming.data)) {
    ...
    if (["permissions", "paths", "checksClaimed", "capabilities", "outputs"].includes(key) && (!Array.isArray(value) || value.length > 64 || value.some(v => typeof v !== "string" || !v.trim() || v.length > 512))) throw new Error(`Invalid ${key}`);
