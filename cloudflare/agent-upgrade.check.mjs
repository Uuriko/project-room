...
import { frozenRecoveryFixture, v8ConnectionBaseline, v9TextBaseline, v10CharterBaseline, v11ReplyBaseline, v12HelpBaseline, v13OfferBaseline, v14InboxBaseline, v15AdoptionBaseline, v16SendBaseline, v17EmailBaseline, v18EmailSourceBaseline, v19EmailExcerptBaseline } from '../scripts/frozen-runtime-fixture.mjs';

for (const [sourceVersion, baseline] of [[8, v8ConnectionBaseline], [9, v9TextBaseline], [10, v10CharterBaseline], [11, v11ReplyBaseline], [12, v12HelpBaseline], [13, v13OfferBaseline], [14, v14InboxBaseline], [15, v15AdoptionBaseline], [16, v16SendBaseline], [17, v17EmailBaseline], [18, v18EmailSourceBaseline], [19, v19EmailExcerptBaseline]]) test(`real Workers v${sourceVersion}→v20 permit replacement, rollback, old-writer refusal and restart`, { timeout: 60000 }, async () => {
      ...
      export class UpgradeRoom {
        ...
        async fetch(request) {
          ...
          if(path==='/rollback') {
            ...
            assert.deepEqual(observed,{version:20,permit:20},'fault occurs after installing the new writer');
          ...
          if(path==='/upgrade') {
            const before=data(), store=current(); assert.equal(version(),20);assert.equal(permit(),0);assert.deepEqual(data(),before);
            ...
            const inboxRequests = [
              {action:'source.save',requestId:'worker-source',sourceId:'private-source',expectedRevision:0,
                data:{adapter:'synthetic',sender:'sender@example.test',recipient:'owner@example.test',subject:'Private sample',
                  paragraphs:['Shared sample paragraph','Excluded private paragraph']}},
              {action:'draft.save',requestId:'worker-draft',sourceId:'private-source',expectedRevision:0,sourceRevision:1,body:'Keep private'}
            ];
            const inboxSaved = inboxRequests.map(req=>store.inbox.apply(f.token,req,f.session.sessionBinding));
            const shareContext=store.inbox.shareContext(f.token,'private-source','commons',f.session.sessionBinding);
            ...
            const preview=store.inbox.sendContext(f.token,'private-source',f.session.sessionBinding).preview;
            const sendRequests=[
              {action:'send.reserve',requestId:'worker-send',sourceId:'private-source',sourceRevision:preview.sourceRevision,draftRevision:preview.draftRevision,previewVersion:preview.previewVersion},
              {action:'send.dispatch',requestId:'worker-dispatch',sourceId:'private-source',sendId:'worker-send',expectedRevision:0},
            ...
            return Response.json({upgraded:true,details,disconnect,receipt:result.receipt,charterCommand,charterSaved,question,asked,answer,answered,helpCommand,helpSaved,helpWithdraw,helpClosed,offerCommand,offerSaved,selectCommand,selectSaved,releaseCommand,releaseSaved,inboxRequests,inboxSaved,sendRequests,sendSaved,cursors,audit:auditRecovery(store)});
          }
          if(path==='/restart') {
            const proof=await request.json(),store=current();assert.equal(version(),20);assert.equal(permit(),0);
            ...
            proof.sendRequests.forEach((req,index)=>assert.deepEqual((req.action==='send.reserve'?store.inbox.apply(f.token,req,f.session.sessionBinding):store.inbox.transport(f.token,req,f.session.sessionBinding)).receipt,proof.sendSaved[index]));
            assert.equal(store.inbox.sends(f.token,'private-source',f.session.sessionBinding).sends[0].status,'accepted');
