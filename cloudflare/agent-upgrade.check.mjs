...
import { frozenRecoveryFixture, v8ConnectionBaseline, v9TextBaseline, v10CharterBaseline, v11ReplyBaseline, v12HelpBaseline, v13OfferBaseline, v14InboxBaseline, v15AdoptionBaseline, v16SendBaseline, v17EmailBaseline } from '../scripts/frozen-runtime-fixture.mjs';

for (const [sourceVersion, baseline] of [[8, v8ConnectionBaseline], [9, v9TextBaseline], [10, v10CharterBaseline], [11, v11ReplyBaseline], [12, v12HelpBaseline], [13, v13OfferBaseline], [14, v14InboxBaseline], [15, v15AdoptionBaseline], [16, v16SendBaseline], [17, v17EmailBaseline]]) test(`real Workers v${sourceVersion}→v18 permit replacement, rollback, old-writer refusal and restart`, { timeout: 60000 }, async () => {
      ...
      export class UpgradeRoom {
        ...
        async fetch(request) {
          ...
          if(path==='/rollback') {
            ...
            assert.deepEqual(observed,{version:18,permit:18},'fault occurs after installing the new writer');
          ...
          if(path==='/upgrade') {
            const before=data(), store=current(); assert.equal(version(),18);assert.equal(permit(),0);assert.deepEqual(data(),before);
          ...
          if(path==='/restart') {
            const proof=await request.json(),store=current();assert.equal(version(),18);assert.equal(permit(),0);
