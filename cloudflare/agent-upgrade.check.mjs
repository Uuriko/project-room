      ...
      export class UpgradeRoom {
        ...
        async fetch(request) {
          ...
          if(path==='/rollback') {
            ...
            assert.deepEqual(observed,{version:24,permit:24},'fault occurs after installing the new writer');
          ...
          if(path==='/upgrade') {
            const before=data(), store=current(); assert.equal(version(),24);assert.equal(permit(),0);assert.deepEqual(data(),before);
          ...
          if(path==='/restart') {
            const proof=await request.json(),store=current();assert.equal(version(),24);assert.equal(permit(),0);
