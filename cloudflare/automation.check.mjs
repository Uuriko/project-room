...
test('Worker upgrades genuine31 and persists shared automation consent across restart', { timeout: 60000 }, async t => {
  ...
  const mf = new Miniflare({ modules: true, script: bundled.outputFiles[0].text, compatibilityDate: '2026-07-30', compatibilityFlags: ['nodejs_compat'],
    durableObjects: { RUN: { className: 'AutomationExperiment', useSQLite: true } } });
  try {
    const response = await mf.dispatchFetch('http://localhost/test');
    assert.equal(response.status, 200, await response.clone().text());
    assert.deepEqual(await response.json(), { upgraded: true, oldWriterBlocked: true, consentPersisted: true });
