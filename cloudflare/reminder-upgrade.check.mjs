...
test('real Workers v7→v15 migration fences a cached legacy adapter, rolls back failures and survives restart', { timeout: 90000 }, async () => {
  const fixture = createAcceptanceFixture(), db = fixture.store.db;
  const persistence = mkdtempSync(join(tmpdir(), 'room-reminder-upgrade-'));
  const schema = db.prepare("SELECT name,type,sql FROM sqlite_master WHERE sql IS NOT NULL AND name NOT GLOB 'writer_v*' AND name NOT GLOB 'private_reminder*' AND name NOT GLOB 'private_inbox*' AND name NOT GLOB 'agent_connection*' ORDER BY rowid").all();
