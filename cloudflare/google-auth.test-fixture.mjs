...
export class GoogleTestRoom extends ProjectRoom {
 async fetch(request) {
  if (new URL(request.url).pathname === "/__verify") {
   const { token, jwk, redirect } = await request.json();
   const google = new GoogleSignIn({ clientId: "123-example.apps.googleusercontent.com", clientSecret: "synthetic-secret", redirectUri: "https://room.example.test/api/auth/google/callback", fetchImpl: async (url, init) => { new Request(url, init); return redirect ? new Response(null, {status: 302, headers: {Location: 'https://untrusted.example/'}}) : Response.json({keys: [jwk]}); } });
