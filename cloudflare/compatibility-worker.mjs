  ...
  async fetch(request) {
    ...
    if (path === '/native') {
      return Response.json([
        ...
        check('node writer function', () => sql.exec('SELECT project_room_writer_v8()').toArray()),
    ...
    if (path === '/initialize') {
      storage.transactionSync(() => {
        ...
        sql.exec('INSERT INTO room_runtime_version VALUES(1,8)');
    ...
    if (path === '/prove') {
      ...
      const writeGuard = storage.transactionSync(() => {
        sql.exec('UPDATE room_runtime_version SET version=9');
