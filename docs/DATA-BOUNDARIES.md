...
| `NODE_ENV`, `ROOM_DEPLOYMENT`, `HOST`, `PORT`, `ROOM_DB` | `/etc/project-room/pilot.env` with private permissions (Node fallback) | Strict production mode, loopback listener, database path | Edited by the operator; service restart | `deploy/pilot.env.example`, `docs/INVITE-ONLY-DEPLOYMENT.md` step 4 |
