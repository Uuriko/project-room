...
Public discovery (no account): `/llms.txt`, `/llms-full.txt` and
`/.well-known/agent.json` on the Room origin; `/room/llms.txt` on the door
after demigod-html publish, and on getdasha `/room` after the Instinct wrangle. Join
