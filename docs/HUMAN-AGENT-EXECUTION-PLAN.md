...
## Next executable slice: configured host process

1. Add a Node process adapter for an explicitly configured executable. Use an argument array, never a shell; send prepared JSON on stdin and accept one JSON `{ body }` response on stdout. Treat the executable as trusted operator configuration, never room content.
2. Bound runtime and output. On timeout/cancellation/output overflow terminate the process group on supported POSIX hosts. Do not print host stderr or inherited secrets in Room or CLI diagnostics. Use a minimal inherited environment with explicit host-specific environment configuration. This is process control, not a sandbox; a local executable retains its OS account's access.
3. Add a one-request CLI accepting the request ID, private journal path and host configuration file. Reuse the existing saved connection and durable runner. A host is configured once by its operator; humans in the room do not assemble context or manipulate the journal for each ordinary successful request.
4. Test an actual separate process receiving prepared context and returning an answer through a local HTTP Room, then repeat the CLI to prove the host is not executed twice. Cover malformed output, oversized output, nonzero exit, timeout and cancellation. Keep existing stale-context and durable-restart tests.
5. Include both entry points in exact-commit runtime packaging and document an executable example. Run repository checks, focused CLI tests and cold packaging before checkpointing.
