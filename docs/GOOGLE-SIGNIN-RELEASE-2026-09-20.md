...
A real previously invited browser session exposed a fourth failure: rotation deleted a slot referenced by immutable share-link join history. Keep only those referenced slots as expired, unauthenticated tombstones; cleanup skips them and the live-slot cap excludes expired rows. A regression proves old-token denial, successful Google-account session rotation, unchanged invitation receipts, and subsequent cleanup after expiry.
