...
- Still open in navigation: a distinct room-list route, delayed-response audit and the remaining copied-link/reload/membership-change combinations. This checkpoint does not claim a complete router redesign.
- Next: distinct room-list navigation and a late-response audit, followed by a focused send-state audit and a scoped shell/Activity contract.
