...
Known baseline: full browser checkpoint previously had 28 failures; the full root suite had an inherited oversized-upload EPIPE failure. Later bounded suites passed, but they do not establish that every old failure is resolved. Keep this distinction in release decisions.
...
- Next: classify the refreshed full-suite baseline and fix core-journey release blockers, then the scoped shell/Activity contract.
