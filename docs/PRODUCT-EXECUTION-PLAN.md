...
- Still open in navigation: the remaining copied-link/membership-change combinations and delayed search, pagination, share and send completions. The targeted response checks do not establish a complete asynchronous-state audit.
- Next: focused send-state and remaining delayed-operation audit, then the scoped shell/Activity contract.
