...
- **Codex / Instinct**: Quiet Focus / return-brief Contributors **read-model only**. Import `contributorsForReturnBrief` (and `attachContributorsToReturnBrief` / `renderContributorsSection`) from `contribution-rollup`. Exact Phase 0 insertion points are in [`contribution-rollup/RETURN-BRIEF-HOOK.md`](../contribution-rollup/RETURN-BRIEF-HOOK.md). Same source Events. No separate scoreboard UI. Optional `proposedById` on work cards is later, not this slice. Do not merge #8/#9 to wire it.
