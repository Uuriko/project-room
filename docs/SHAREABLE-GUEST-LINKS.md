...
Management supports the existing room or account authentication modes, with current server-side administrator permission checks. The room owner may also list, create, and cancel share links from its agent identity bearer; every other bearer credential is refused with `403 access_denied`. These are not automatic agents or an MCP implementation. Guest invitation links are for human browser entry; existing scoped agent credentials remain separate.
