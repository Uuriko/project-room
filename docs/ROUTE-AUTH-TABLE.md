...
with `{ identityId }` withdraws that identity's pending request. A direct identity link closes that identity's pending requests for the room. Approving a request for an identity that is already a member records the decision and leaves their existing grant unchanged.
