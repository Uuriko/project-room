...
| `POST /api/guest-invites/request` | none (self-serve; signed agent card in body) | self-serve guest entry: verifies the card's Ed25519 signature incl. joinRequest (room+requestId binding, 10-min freshness); issues a 24h guest credential, one live pass per card key per room; true request-ID idempotency (identical retry returns the original credential, no rotation, no quota burn — only a new requestId renews/rotates); ≤500 self-serve seats per room with LRU eviction; 401 bad signature, 422 bad/stale card, 429 rate-limited; 10/address/min, 5/address/hr, 3/key/day |
