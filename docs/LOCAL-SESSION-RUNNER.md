...
The automated acceptance cases use human and agent requesters, an authenticated agent executor and real scripted processes. They are not proof of model reasoning or production agent interoperability. For execution without work, set `workItemId: null` and `answerRequestId` to the selected request ID. This uses the durable request-run claim instead of creating a Work Item. Use expected revision 0 for a first attempt; subsequent deliberately authorized attempts require the inspected terminal run revision and a new run ID. The existing local connection remains agent-only; human participants can ask, clarify and cancel. This does not broaden the agent credential contract to human credentials. Shared recurring automation and in-app run initiation remain open.

Work-free runs use the recorded deadline as an additional local runtime limit; clock skew may conservatively refuse a run. Expired or uncertain ownership never permits automatic takeover. Multiple local configuration directories still compete for the same server-side claim. Request success only permits the separate pinned-context answer attempt; it is not approval or proof of outside effects.

Selected-request runs now pin the inspected request context and room-instructions revision. Before claiming and launching, and during the existing authenticated session polling, the request must remain open, assigned to this agent and work item, and unchanged. Cancellation, an answer by another session, a clarification or changed room instructions stops the local process with `request_changed`; unfinished output is not posted. This is polling-based cancellation, not instantaneous rollback of effects the program already performed. The final answer still uses the original context basis and can be refused if a change races the last poll. Never automatically restart with refreshed context; inspect the change and deliberately authorize a new run.
